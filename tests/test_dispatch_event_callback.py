...
in the chat log with a `[specialty]` prefix. The callback contract is:
  ...
  - When set, called with (specialty, dispatch_id, sub_turn, kind, content)
    for each sub-agent event
...
def test_emit_dispatch_event_noop_when_callback_unset(tmp_path, monkeypatch):
    ...
    # Must not raise
    sess.emit_dispatch_event("ad", "test-dispatch-id", 1, "tool_call", "nmap_scan {...}")


def test_emit_dispatch_event_invokes_callback(tmp_path, monkeypatch):
    """When the callback is set, every emit forwards (specialty, dispatch_id, sub_turn, kind, content)."""
    monkeypatch.setenv("REVERSER_TARGETS_DIR", str(tmp_path))
    from reverser.profiles import get_profile
    from reverser.agent_session import AgentSession
    ...
    received: list[tuple[str, str, int, str, str]] = []
    sess.on_dispatch_event = lambda specialty, dispatch_id, sub_turn, kind, content: received.append(
        (specialty, dispatch_id, sub_turn, kind, content)
    )

    sess.emit_dispatch_event("ad", "test-dispatch-id", 1, "tool_call", "nmap_scan {target:10.10.10.5}")
    sess.emit_dispatch_event("webpentest", "test-dispatch-id", 1, "tool_result", "200 OK")
    sess.emit_dispatch_event("ad", "test-dispatch-id", 1, "thinking", "Should I enumerate SMB?")
    sess.emit_dispatch_event("ad", "test-dispatch-id", 1, "text", "Found a domain controller.")

    assert received == [
        ("ad", "test-dispatch-id", 1, "tool_call", "nmap_scan {target:10.10.10.5}"),
        ("webpentest", "test-dispatch-id", 1, "tool_result", "200 OK"),
        ("ad", "test-dispatch-id", 1, "thinking", "Should I enumerate SMB?"),
        ("ad", "test-dispatch-id", 1, "text", "Found a domain controller."),
    ]
...
def test_emit_dispatch_event_swallows_callback_exceptions(tmp_path, monkeypatch):
    ...
    def boom(specialty: str, dispatch_id: str, sub_turn: int, kind: str, content: str) -> None:
...
def test_emit_dispatch_event_callback_cleared_between_turns(tmp_path, monkeypatch):
    ...
    sess.emit_dispatch_event("ad", "test-dispatch-id", 1, "text", "first turn")

    # TUI clears the callback in the `finally` of _run_agent
    sess.on_dispatch_event = None
    sess.emit_dispatch_event("ad", "test-dispatch-id", 1, "text", "should not be captured")

    assert received == [("ad", "test-dispatch-id", 1, "text", "first turn")]
